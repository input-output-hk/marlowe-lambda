...
module Language.Marlowe.Lambda.Sign (
  sign
) where
...
import qualified Cardano.Api as C (IsShelleyBasedEra, PaymentExtendedKey, PaymentKey, ShelleyWitnessSigningKey(WitnessPaymentExtendedKey, WitnessPaymentKey), SigningKey, Tx, TxBody, getTxId, signShelleyTransaction)
